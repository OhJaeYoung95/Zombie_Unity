...
public class EnemySpawner : MonoBehaviour {
    ...
    private void CreateEnemy(float intensity) 
    {
        ...
        var color = Color.Lerp(Color.white, Color.red, intensity);
        var scaleSize = Mathf.Lerp(scaleMin, scaleMax, intensity);
        var scale = new Vector3(1f, scaleSize, 1f);
        var point = spawnPoints[Random.Range(0, spawnPoints.Length)];

        var enemy = Instantiate(enemyPrefab, point.position, point.rotation);
        enemy.Setup(health, damage, speed, scale, color);
        enemies.Add(enemy);
