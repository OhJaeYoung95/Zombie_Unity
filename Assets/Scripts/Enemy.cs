...
public class Enemy : LivingEntity 
{
    ...
    public void Setup(float newHealth, float newDamage, float newSpeed, Vector3 scale ,Color skinColor) 
    {
    ...
    void TryAttack(Collider other)
    {
        ...
        if (Time.time > lastAttackTime + timeBetAttack && other.gameObject == targetEntity.gameObject)
        {
            ...
            other.GetComponent<PlayerHealth>().OnDamage(damage, other.transform.position, dir.normalized);
        }


        //{
        //    lastAttackTime = Time.time;
        //    var ray = new Ray(transform.position + Vector3.up, transform.forward * 5);
        //    if (Physics.Raycast(ray, out RaycastHit hitInfo, 5))
        //    {
        //        var target = hitInfo.collider.GetComponent<IDamageable>();
        //        if (target != null)
        //        {
        //            target.OnDamage(damage, hitInfo.point, hitInfo.normal);
        //        }
        //    }
        //}
    }
